export default function Packages() {
  ...
  return (
    <div
      className="relative flex flex-col h-full lg:h-screen px-16 py-16 gap-4 bg-[#2D2D2D]"
      id="packages"
    >
      <svg
      ...
      <div className="z-5 text-4xl lg:text-6xl font-bold text-white">
        <span className="text-[#64B386]">HR Outsource</span>{" "}
        ที่เหมาะกับธุรกิจของคุณ
      </div>
      <div className="flex flex-col md:flex-row h-full w-full z-5 justify-center content-center gap-6 md:gap-10">
        {packages.map((pkg, index) => (
          <div
            key={index}
            className="group flex flex-col w-full lg:w-4/12 px-6 py-4 justify-center content-center gap-4 border-4 border-[#4CAF50] lg:border-gray-400 rounded-4xl bg-white/10 backdrop-blur-md cursor-pointer
            hover:border-[#4CAF50] hover:scale-105 transition-border duration-300"
          >
            <div className="text-[#95E999] lg:text-[#64B386] text-2xl lg:text-4xl font-bold transition-colors duration-300 lg:group-hover:text-[#95E999]">
              {pkg.title}
            </div>
            <div className="font-bold text-xl lg:text-3xl text-white lg:text-white/80 transition-opacity duration-300 group-hover:text-white">
              {pkg.price} บาท / เดือน
            </div>
            <div className="text-xl lg:text-3xl text-white/80 lg:text-white/30 transition-opacity duration-300 group-hover:text-white/80">
              {pkg.for}
            </div>
              ...
              <div className="flex flex-col w-full">
                {pkg.description.map((desc, idx) => (
                  <div key={idx} className="flex flex-row items-center gap-2">
                    ...
                    <span className="text-white/30 text-base lg:text-xl">
                      {desc}
                    </span>
                  </div>
                ))}
